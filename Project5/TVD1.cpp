...
void TVD1(double Q[][3], double F[][3], int N, double cfl, double dt, double dx)
{
	int i, j, k;
	...
	for (i = 1; i <= N - 1; i++)
	{
		for (j = 0; j <= 2; j++)
		{
			/*	sum = 0;
				for (k = 0; k <= 2; k++)
				{
					sum = sum + psai[i][k] * R[i][k][j];
				}*/
				//NF[i][j] = 0.5 * (F[i][j] + F[i + 1][j] + sum);
			NF[i][j] = 0.5 * (F[i][j] + F[i + 1][j] + (g[i][j] + g[i + 1][j]) / r - (Qk(r * lamda[i][j] + GAMMA[i][j]) * (Q[i + 1][j] - Q[i][j])) / r);
		}
	}
	for (j = 0; j <= 2; j++)
	{
